...
from utils import get_post_by_post_id, get_posts_by_user, get_comments_by_post_id, search_for_posts

get_posts_by_user_parameters = [
    ("leo", [
        {
            "poster_name": "leo",
...
@pytest.mark.parametrize("name, posts_list", get_posts_by_user_parameters)
def test_get_posts_by_user(name, posts_list):
    assert get_posts_by_user(name) == posts_list
